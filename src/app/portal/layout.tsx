...
import { LayoutDashboard, CalendarPlus, History, UserCircle, Users, MessageSquare, Send, CreditCard, FolderOpen } from 'lucide-react'
...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  const TIER_LABEL: Record<string, string> = { free: 'Free', starter: 'Starter Plan', advantage: 'Advantage Plan' }
  ...
  const sidebar = (
    <>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        <NavLink href="/portal/documents"      icon={FolderOpen}      label="My Documents" />
  ...
  return (
    <DashboardShell sidebar={sidebar} banner={banner}>
      {children}
    </DashboardShell>
  )
}
