...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .eq('client_id', user.id) // RLS: only return attachments sent to this client
    .order('created_at', { ascending: true })

  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  // Generate signed URLs and group by message_id
  ...
  await Promise.all(
    (data ?? []).map(async (a) => {
      ...
      const item = { id: a.id, file_name: a.file_name, file_size: a.file_size, mime_type: a.mime_type, url: signed?.signedUrl ?? null }
